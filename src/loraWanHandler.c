...
static void _lora_setup(void)
{
	...
	status_leds_slowBlink(led_ST3); // Led the green led blink slowly while we are setting up LoRa
        ...
        */
        ...
        else
        {
            if (rc == LORA_MAC_RX)
            {
                ...
                printf("Upload Message OK - \n\tDOWN LINK: from port: %d with %d bytes received!", _downlink_payload.portNo, _downlink_payload.len);
                ...
                if (_downlink_payload.len == 12)
                {
                    ...
                    minTemp_ptr = malloc(sizeof(uint16_t));
                    maxTemp_ptr = malloc(sizeof(uint16_t));
                    ...
                    *maxTemp_ptr = (_downlink_payload.bytes[10] << 8) | _downlink_payload.bytes[11];
                    ...
                    updateConfiguration(*minCO2_ptr, *maxCO2_ptr, *minTemp_ptr, *maxTemp_ptr, *minHum_ptr, *maxHum_ptr);
